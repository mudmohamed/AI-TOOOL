...
class DerivWebSocketService {
  ...
  private processVirtualContractsOnTick(_symbol: string, _quote: number, _forceSettlement = false): void {
    // Local contract settlement is permanently disabled.
    // Any stale local practice contracts are discarded and never reported as won/lost.
    if (this.virtualOpenContracts.size) this.virtualOpenContracts.clear();
  }
...
// they do not alter strategy, stake, target, recovery, or settlement logic.
  ...
  private placeVirtualPracticeContract(params: PlaceContractParams): boolean {
    ...
    this.notifyHandlers({
      ...
      stage: 'preflight',
      error: 'Local simulated contracts are disabled. Connect a genuine Deriv Demo or Real account.',
    });
    return false;
  }
