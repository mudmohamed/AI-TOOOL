...
import { MarketAnalysis, AutoMatchesSignal, TradeRecord, AutoMatchesConfig, DerivAccountInfo } from '../types';
import {
...
export const DeepScanAutoMatches: React.FC<DeepScanAutoMatchesProps> = ({
  ...
  const [internalConfig, setInternalConfig] = useState<AutoMatchesConfig>(() => {
    ...
    return {
      ...
      nextTradeCondition: 'SAME_LOSS_RECOVERY',
      martingaleFactor: 1.15,
      restartOnError: true,
      executionSpeed: 'TURBO',
      targetStrategy: 'MARKOV_TRANSITION',
      contractMode: 'MATCHES',
      onlyWhenSignalConfirmed: false,
      stopConditionMode: 'NEVER_STOP_UNTIL_MANUAL',
      targetProfitPercentage: 100,
    };
  });
  ...
  // Dynamic next stake calculation matching DBot XML logic & strict capital preservation
  const calculateNextStake = (): number => {
    ...
    const maxCap = activeConfig.maxStakeCap && activeConfig.maxStakeCap > 0 ? activeConfig.maxStakeCap : initialAmt * 3;

    // Strict Fixed Stake Mode: never increase beyond user's chosen price!
    if (activeConfig.fixedStakeMode || activeConfig.nextTradeCondition === 'FIXED_STAKE') {
      return initialAmt;
    }
    ...
    let calculated = initialAmt;
    if (activeConfig.nextTradeCondition === 'SAME_LOSS_RECOVERY') {
      calculated = Math.max(initialAmt, (sessionStats.cumulativeLoss + winAmt) / 8.5);
    } else if (activeConfig.nextTradeCondition === 'RESET_ON_WIN') {
      calculated = initialAmt;
    } else {
      // Default: Martingale
      const factor = activeConfig.martingaleFactor || 1.15;
      calculated = initialAmt * Math.pow(factor, sessionStats.consecutiveLosses);
    }

    return Number(Math.min(maxCap, Math.min(maxLoss, Math.max(initialAmt, calculated))).toFixed(2));
  };

  const nextCalculatedStake = calculateNextStake();

  const handleManualTradeNow = () => {
    let contractType: 'MATCHES' | 'DIFFERS' | 'OVER' | 'UNDER' =
      activeConfig.contractMode === 'DIFFERS' || activeConfig.targetStrategy === 'COLD_DIFFERS'
        ? 'DIFFERS'
        : activeConfig.contractMode === 'OVER_UNDER'
        ? 'UNDER'
        : 'MATCHES';
    ...
    let payoutRate = 8.342857;

    if (contractType === 'DIFFERS') {
      const cold = findBestDiffersTarget(activeMarketTicks?.digits || [], currentLastDigit);
      targetDigit = activeConfig.customTargetDigit !== undefined ? activeConfig.customTargetDigit : cold.targetDigit;
      payoutRate = 1.095;
    } else if (contractType === 'UNDER') {
      targetDigit = 7;
      payoutRate = 1.40;
    } else if (activeConfig.targetStrategy === 'MARKOV_TRANSITION') {
      const sig = findBestAutoMatchesTarget(
        currentSymbol,
        activeMarketAnalysis?.displayName || currentSymbol,
        activeMarketTicks?.digits || [],
        activeMarketAnalysis?.digitStats || [],
        currentLastDigit
      );
  ...
  return (
    <div id="deep-scan-auto-matches" className="space-y-6">
      ...
      {/* TARGET 100% GOAL TRACKER & CONTINUOUS TRADING SYSTEM */}
      {(() => {
        const targetProfitGoal = activeConfig.expectedProfit || 20.0;
        const currentNetProfit = sessionStats.netProfit;
        const progressPercent = Math.min(
          100,
          Math.max(0, Number(((currentNetProfit / targetProfitGoal) * 100).toFixed(1)))
        );
        const isTarget100Reached = currentNetProfit >= targetProfitGoal && targetProfitGoal > 0;
        const stopMode = activeConfig.stopConditionMode || 'NEVER_STOP_UNTIL_MANUAL';
        const isNeverStopMode = stopMode === 'NEVER_STOP_UNTIL_MANUAL';
        const isTargetOnlyMode = stopMode === 'ONLY_MANUAL_OR_TARGET';

        return (
          <div className="p-4 sm:p-5 rounded-2xl bg-gradient-to-r from-slate-900 via-slate-950 to-slate-900 border border-slate-800 shadow-xl space-y-3">
            <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-3">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <div className="text-sm sm:text-base font-black text-white font-mono flex items-center gap-2 flex-wrap">
                    <span>TARGET 100% GOAL TRACKER</span>
                    <span
                    ...
                    {isNeverStopMode && (
                      <span className="px-2 py-0.5 rounded-full text-[10px] font-bold bg-amber-500/20 text-amber-300 border border-amber-500/40">
                        ⚡ NON-STOP CONTINUOUS RUN
                      </span>
                    )}
                  </div>
                  <div className="text-xs text-slate-400 mt-0.5">
                    Target Goal: <strong className="text-cyan-300 font-mono">${targetProfitGoal.toFixed(2)} USD</strong> • Net P/L:{' '}
                    <strong
                      className={`font-mono ${
              ...
              {/* Stop Condition Policy Selector */}
              <div className="flex flex-wrap items-center gap-1.5 p-1 rounded-xl bg-slate-950 border border-slate-800 text-xs font-mono">
                <button
                  ...
                  title="Bot runs continuously without stopping for targets or losses until you click STOP"
                >
                  <Zap className="w-3.5 h-3.5" />
                  <span>Never Stop (Only When I Stop It)</span>
                </button>
                <button
                  type="button"
                  onClick={() =>
                ...
                <button
                  ...
                  className={`px-3 py-2 rounded-lg font-bold transition cursor-pointer ${
                    stopMode === 'STOP_ON_MAX_LOSS'
                      ? 'bg-rose-600 text-white shadow-md'
                      : 'text-slate-500 hover:text-slate-300'
                  }`}
                >
                  Stop on Max Loss (${(activeConfig.maxAcceptableLoss || 50).toFixed(0)})
                </button>
              </div>
            </div>
            ...
            <div className="space-y-1.5 pt-1">
              <div className="w-full h-4 rounded-full bg-slate-950 border border-slate-800 overflow-hidden p-0.5 relative">
                <div
                  className={`h-full rounded-full transition-all duration-500 ${
                    isTarget100Reached
                      ? 'bg-gradient-to-r from-emerald-500 to-teal-400 shadow-[0_0_15px_rgba(16,185,129,0.8)]'
                      : 'bg-gradient-to-r from-cyan-600 via-teal-500 to-emerald-400 shadow-[0_0_10px_rgba(6,182,212,0.6)]'
                  }`}
                  style={{ width: `${Math.max(2, progressPercent)}%` }}
                />
              </div>
              <div className="flex flex-col sm:flex-row sm:items-center justify-between text-[11px] font-mono gap-1">
                <span>
                  {isNeverStopMode ? (
                    <span className="text-emerald-400 font-bold flex items-center gap-1">
                      <Zap className="w-3.5 h-3.5 text-amber-400" />
                      Continuous Non-Stop Engine: The bot will NEVER stop on its own • Runs trade after trade until you click STOP!
                    </span>
                  ) : isTargetOnlyMode ? (
                    <span className="text-cyan-300">
                      Target 100% Mode: Bot runs until Target Goal is hit or you click STOP.
                    </span>
                  ) : (
                    <span className="text-amber-400">
                      Max Loss Protection: Bot will halt if drawdown hits ${(activeConfig.maxAcceptableLoss || 50).toFixed(2)}
                    </span>
                  )}
                </span>
                <span className="font-bold text-white shrink-0">
                  {isTarget100Reached
                    ? '🎯 100% TARGET REACHED'
                    : `${(100 - progressPercent).toFixed(1)}% remaining to 100%`}
                </span>
              </div>

              {/* 100% Won Profit Vault Protection Bar */}
              <div className="p-3 rounded-xl bg-emerald-950/40 border border-emerald-500/40 flex flex-col sm:flex-row sm:items-center justify-between gap-2.5">
                <div className="flex items-center gap-2">
                  ...
                  <div className="font-mono">
                    <div className="text-xs font-black text-white flex items-center gap-1.5">
                      <span>100% Won Profit Shield</span>
                      <span className="px-1.5 py-0.2 rounded bg-emerald-500/20 text-emerald-300 text-[9px] font-bold">
                        ACTIVE
                      </span>
                    </div>
                    <div className="text-[10px] text-slate-400">
                      Accumulated won profits are shielded so gains are never lost back to the market.
                    </div>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-2 font-mono">
                  {vaultedProfit > 0 && (
                    <div className="px-2.5 py-1.5 rounded-lg bg-slate-900 border border-slate-800 text-xs text-emerald-400 font-bold">
                      Vaulted: ${vaultedProfit.toFixed(2)}
                    </div>
                  )}
                  {onVaultWonProfit && currentNetProfit > 0 && (
                    <button
                      type="button"
                      onClick={onVaultWonProfit}
                      className="px-3 py-1.5 rounded-lg bg-gradient-to-r from-emerald-500 to-teal-400 hover:from-emerald-400 hover:to-teal-300 text-slate-950 font-black text-xs transition cursor-pointer shadow-md active:scale-95"
                    >
                      🏦 Lock +${currentNetProfit.toFixed(2)} in Vault
                    </button>
                  )}
                </div>
              </div>
      ...
      <div className="p-4 sm:p-5 rounded-2xl border border-slate-800 bg-slate-900/90 shadow-xl space-y-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <div className="p-3.5 rounded-xl bg-slate-950 border border-slate-800 space-y-2">
            <div className="flex items-center justify-between text-[10px] font-mono uppercase text-slate-400 font-bold">
              <span>Expected Profit (Target 100%)</span>
            ...
            <div className="text-[10px] text-slate-500 font-mono">
              The 100% Target Goal. The bot runs continuously until stopped by you or this target is 100% reached.
            </div>
          </div>
          ...
          <div className="p-3.5 rounded-xl bg-slate-950 border border-slate-800 space-y-2">
            <div className="flex items-center justify-between text-[10px] font-mono uppercase text-slate-400 font-bold">
              ...
              <span className="text-rose-400 font-bold">
                ${(activeConfig.maxAcceptableLoss || 50).toFixed(2)}
                {(activeConfig.stopConditionMode || 'ONLY_MANUAL_OR_TARGET') === 'ONLY_MANUAL_OR_TARGET' && (
                  <span className="ml-2 text-[9px] text-emerald-400 font-normal lowercase">(bypassed)</span>
                )}
              </span>
            </div>
            <div className="flex items-center gap-1.5">
            ...
            <div className="text-[10px] font-mono">
              {(activeConfig.stopConditionMode || 'ONLY_MANUAL_OR_TARGET') === 'ONLY_MANUAL_OR_TARGET' ? (
                <span className="text-emerald-400/90">
                  Continuous Mode: Stop loss is disabled. Bot only stops by you or when Target hits 100%.
                </span>
              ) : (
                <span className="text-rose-400/90">
                  Circuit Breaker active: Trading will halt if drawdown reaches ${(activeConfig.maxAcceptableLoss || 50).toFixed(2)}.
                </span>
              )}
            </div>
          </div>
          ...
          <div className="p-3.5 rounded-xl bg-slate-950 border border-slate-800 space-y-2">
            <div className="text-[10px] font-mono uppercase text-slate-400 font-bold flex items-center justify-between">
              <span>Next Trade Condition</span>
              {activeConfig.nextTradeCondition === 'FIXED_STAKE' && (
            ...
            <select
              ...
              onChange={(e) =>
                handleConfigChange({
                  ...
                  fixedStakeMode: e.target.value === 'FIXED_STAKE',
                })
              }
            ...
            >
              <option value="FIXED_STAKE">🛡️ Strict Fixed Stake (Never Increase - Account Protector)</option>
              ...
              <option value="RESET_ON_WIN">Reset on Win (Base Stake)</option>
            </select>
            <div className="text-[10px] text-slate-500 font-mono">
              {activeConfig.nextTradeCondition === 'FIXED_STAKE'
                ? 'Guarantees stake will NEVER exceed your chosen initial price.'
                : 'Due to 8.34x payout, 1 win recovers multiple prior losses easily.'}
            </div>
          </div>
        </div>

        {/* Capital Protection & Anti-Blowout Settings Row */}
        <div className="p-3.5 rounded-xl bg-slate-950/80 border border-emerald-500/30 grid grid-cols-1 sm:grid-cols-3 gap-3 font-mono">
          <div>
            <label className="text-[10px] font-bold text-amber-400 uppercase tracking-wider block mb-1">
              Strict Stake Ceiling (Max Cap)
            </label>
            <div className="flex items-center gap-1.5">
              {[0.5, 1.0, 2.0, 5.0].map((cap) => (
                <button
                  key={cap}
                  type="button"
                  onClick={() => handleConfigChange({ ...activeConfig, maxStakeCap: cap })}
                  className={`flex-1 py-1 rounded-lg text-xs font-bold transition cursor-pointer ${
                    (activeConfig.maxStakeCap || 1.0) === cap
            ...
            <span className="text-[9px] text-slate-400 block mt-1">
              Hard limit: bot will NEVER place a trade higher than this.
            </span>
          </div>
          ...
          <div>
            ...
            <div className="flex items-center gap-1.5">
              {[2, 3, 5, 8].map((allowed) => (
                <button
                  key={allowed}
                  type="button"
                  onClick={() => handleConfigChange({ ...activeConfig, maxAllowedLosses: allowed })}
                  className={`flex-1 py-1 rounded-lg text-xs font-bold transition cursor-pointer ${
                    (activeConfig.maxAllowedLosses || 3) === allowed
            ...
            <span className="text-[9px] text-slate-400 block mt-1">
              Halts trading automatically if reached, preserving your account.
            </span>
          </div>
          ...
          <div className="flex flex-col justify-between">
            ...
            <button
              ...
              onClick={() =>
                handleConfigChange({
                  ...activeConfig,
                  onlyWhenSignalConfirmed: !activeConfig.onlyWhenSignalConfirmed,
                })
              }
              className={`w-full py-1.5 px-3 rounded-lg text-xs font-bold transition flex items-center justify-center gap-2 cursor-pointer ${
                activeConfig.onlyWhenSignalConfirmed !== false
            ...
            >
              ...
              <span>
                {activeConfig.onlyWhenSignalConfirmed !== false
                  ? 'Active (Confirmed Only)'
                  : 'Disabled (Every Tick)'}
              </span>
            </button>
            <span className="text-[9px] text-slate-400 block mt-1">
              Prevents blind wrong trades; only trades on statistical Markov edge.
            </span>
          </div>
        </div>

        {/* Execution Speed & Cadence Selector */}
        <div className="p-3.5 rounded-xl bg-slate-950 border border-slate-800 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          <div className="space-y-0.5">
            <div className="text-xs font-bold text-white font-mono flex items-center gap-1.5">
              ...
              Trading Speed &amp; Cadence:
            </div>
            <div className="text-[11px] text-slate-400">
              Control dispatch latency between live trades:
            </div>
          </div>
          ...
          <div className="flex items-center gap-2 flex-wrap font-mono">
            <button
              type="button"
              onClick={() => handleConfigChange({ ...activeConfig, executionSpeed: 'TURBO' })}
              className={`px-3 py-1.5 rounded-lg text-xs font-bold transition cursor-pointer flex items-center gap-1.5 ${
                activeConfig.executionSpeed === 'TURBO'
                  ? 'bg-amber-400 text-slate-950 shadow ring-2 ring-amber-300 font-black'
                  : 'bg-slate-900 text-slate-400 hover:text-white border border-slate-800'
              }`}
            >
              <Zap className="w-3 h-3 fill-current" />
              <span>TURBO (Ultra-Fast 0ms)</span>
            </button>

            <button
              type="button"
              onClick={() => handleConfigChange({ ...activeConfig, executionSpeed: 'FAST' })}
              className={`px-3 py-1.5 rounded-lg text-xs font-bold transition cursor-pointer ${
                activeConfig.executionSpeed === 'FAST'
                  ? 'bg-emerald-500 text-slate-950 shadow ring-1 ring-emerald-300 font-bold'
                  : 'bg-slate-900 text-slate-400 hover:text-white border border-slate-800'
              }`}
            >
              FAST (40ms Pulse)
            ...
            <button
              type="button"
              onClick={() => handleConfigChange({ ...activeConfig, executionSpeed: 'NORMAL' })}
              className={`px-3 py-1.5 rounded-lg text-xs font-bold transition cursor-pointer ${
                activeConfig.executionSpeed === 'NORMAL'
        ...
        <div className="p-3.5 rounded-xl bg-slate-950 border border-slate-800 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          <div className="space-y-0.5">
            ...
            <div className="text-[11px] text-slate-400">
              Select trade contract type on live Deriv:
            </div>
          </div>
          ...
          <div className="flex items-center gap-2 flex-wrap">
            <button
              type="button"
              onClick={() => handleConfigChange({ ...activeConfig, contractMode: 'MATCHES' })}
              className={`px-3 py-1.5 rounded-lg text-xs font-mono font-bold transition cursor-pointer ${
                activeConfig.contractMode === 'MATCHES' || !activeConfig.contractMode
                  ? 'bg-emerald-500 text-slate-950 shadow ring-1 ring-emerald-300'
                  : 'bg-slate-900 text-slate-400 hover:text-white border border-slate-800'
              }`}
            >
              DIGITMATCH (834% Payout)
            </button>

            <button
              type="button"
              onClick={() => handleConfigChange({ ...activeConfig, contractMode: 'DIFFERS', targetStrategy: 'COLD_DIFFERS' })}
              className={`px-3 py-1.5 rounded-lg text-xs font-mono font-bold transition cursor-pointer flex items-center gap-1.5 ${
                activeConfig.contractMode === 'DIFFERS'
                  ? 'bg-cyan-500 text-slate-950 shadow ring-1 ring-cyan-300 font-black'
                  : 'bg-slate-900 text-slate-400 hover:text-white border border-slate-800'
              }`}
            >
              <ShieldCheck className="w-3.5 h-3.5" />
            ...
            <button
              type="button"
              onClick={() => handleConfigChange({ ...activeConfig, contractMode: 'OVER_UNDER' })}
              className={`px-3 py-1.5 rounded-lg text-xs font-mono font-bold transition cursor-pointer ${
                activeConfig.contractMode === 'OVER_UNDER'
                  ? 'bg-indigo-500 text-white shadow ring-1 ring-indigo-300'
                  : 'bg-slate-900 text-slate-400 hover:text-white border border-slate-800'
              }`}
            >
              UNDER 7 (70% Win Rate)
        ...
        <div className="p-3.5 rounded-xl bg-slate-950 border border-slate-800 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          <div className="space-y-0.5">
            ...
            <div className="text-[11px] text-slate-400">
              Select how the predicted digit is determined for each contract:
            </div>
          </div>
          ...
          <div className="flex items-center gap-2 flex-wrap">
            <button
              ...
              className={`px-3 py-1.5 rounded-lg text-xs font-mono font-bold transition cursor-pointer ${
                activeConfig.targetStrategy === 'MARKOV_TRANSITION' || !activeConfig.targetStrategy
            ...
            <button
              type="button"
              onClick={() => handleConfigChange({ ...activeConfig, targetStrategy: 'COLD_DIFFERS', customTargetDigit: undefined })}
              className={`px-3 py-1.5 rounded-lg text-xs font-mono font-bold transition cursor-pointer ${
                activeConfig.targetStrategy === 'COLD_DIFFERS'
                  ? 'bg-cyan-600 text-white shadow ring-1 ring-cyan-300'
                  : 'bg-slate-900 text-slate-400 hover:text-white border border-slate-800'
              }`}
            >
              Cold Outlier (Differs)
            </button>
        ...
        <div className="p-3 rounded-xl bg-slate-950 border border-slate-800 flex items-center justify-between gap-3 text-xs font-mono">
          <label className="flex items-center gap-2 cursor-pointer text-slate-300 hover:text-white select-none">
            <input
              type="checkbox"
              checked={Boolean(activeConfig.onlyWhenSignalConfirmed)}
              onChange={(e) => handleConfigChange({ ...activeConfig, onlyWhenSignalConfirmed: e.target.checked })}
              className="w-4 h-4 rounded bg-slate-900 border-slate-700 text-emerald-500 focus:ring-0 cursor-pointer"
            />
            <span>Wait for Confirmed Statistical Signal (Skip low-probability ticks to maximize win rate)</span>
          </label>
          <span className="text-[10px] text-emerald-400 font-bold hidden sm:inline">
            {activeConfig.onlyWhenSignalConfirmed ? 'Gated: High-Prob Only' : 'Continuous Stream'}
          </span>
        </div>
      </div>
